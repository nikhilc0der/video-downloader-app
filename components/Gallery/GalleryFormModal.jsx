...
export default function GalleryFormModal({
  ...
  const [collectionValue, setCollectionValue] = useState(collectionName);
  ...
  const handleSubmit = () => {
    if (collectionValue) {
      let elementData = element ? [...element] : 0;
      dispatch(
        UpdateCollection({
          id: id,
          collectionName: collectionValue,
          collectionArray: elementData,
        })
      );
      setRename(false);
      goBackHandler();
    ...
    <>
      <View
        className={`absolute top-0 w-full h-full opacity-50 bg-black ${
          isRename ? "flex-col" : "hidden"
        }`}
        style={{ zIndex: 101 }}
      ></View>

      <View style={{ zIndex: 110 }}>
        <Modal visible={isRename} animationType="slide" transparent={true}>
